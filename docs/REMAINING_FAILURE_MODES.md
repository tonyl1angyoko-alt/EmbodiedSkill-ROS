...
| Fresh safety observation changes after guard and before ROS action transition | Freshness bounds age but supplies no atomic lease/interlock across asynchronous middleware | Architectural | No; ROS L1 executes AGV motion after arm safety becomes false | Controller-side invariant/interlock, state leases with invalidation, or transactional command admission |
...
| JAKA execution remains unverified | Vendor source/ELF exists locally, but bridge/interfaces are not built or active and no hardware is connected | Platform/deployment limitation | N/A | Build dependencies, supervise hardware validation, calibrate state/stop semantics |
